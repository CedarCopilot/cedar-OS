...
agentInputContext handles the context that we pass into the agent. It stringifies the context & editor through `stringifyInputContext` and `compileAdditionalContext` and helps generally manage the context we're giving the agent.
