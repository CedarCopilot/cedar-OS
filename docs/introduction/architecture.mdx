...
Let's say you want an AI to be able to read and write different parts of your react app. For example, you're making an AI-native email, and you want it to be able to help with drafts, categorise emails, and

1. **Component-scoped State**: A traditional react app typically has state inside of the component that uses it. This totally makes sense, and makes for a really great developer experience where you can combine logic and rendering together!
...
1. **State Portals**: Each "RegisterCedarState" acts like a portal the agent can reach the specific state. You don't need to worry about contexts, their scopes, and most importantly the fact that if one context field changes, everything that subscribes to it changes.
...
At the base, we provide optimized hooks that connect directly to our Zustand store:
...
These hooks abstract away the complexity of state management, agent communication, and real-time updates while maintaining full type safety and performance optimization. You can of course create your own hooks that plug into the Zustand store.
...
At the top layer, we provide complete, production-ready AI experiences. We can provide sophisticated solutions like this because you own the code, and so you can customise it however you want.

- **`<FloatingCedarChat />`**: Floating chat interface that can be summoned anywhere. This combines `<ChatInput />`, `<ContextBadge />`, `<ChatMessages />`, and positional Containers like `<FloatingContainer />`
- **`SidePanelCedarChat`**: Sidebar AI assistant for complex applications
...
2. **Customize Gradually**: Replace individual components as your needs evolve
