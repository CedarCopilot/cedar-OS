...
Cedar-OS provides comprehensive Zod schemas for runtime validation and enhanced type safety. These schemas are available for import and can be used to validate backend requests:

```typescript
// Import Zod schemas from cedar-os
import {
	BaseParamsSchema,
	MastraParamsSchema,
	CustomParamsSchema,
...
// Create typed schemas for custom backends
const MyCustomSchema = MastraParamsSchema<MyContextType, MyCustomFields>();

// Runtime validation
const validatedParams = MyCustomSchema.parse(incomingRequest);
```

The schema system supports:

- **Generic typing** for custom additional context and fields
- **Provider-specific schemas** for different backend types
- **Runtime validation** with full TypeScript integration
- **Factory functions** for creating typed schemas
