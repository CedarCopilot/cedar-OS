---
title: 'How Responses are Processed (and Custom Processing)'
description: 'Learn how Cedar deals with all structured objects returned from the backend.'
---

Cedar allows you to completely customize how structured responses from your agent backend are processed. This is the system through which messages are added to the chat, state actions are executed, or any decisions are made on how to deal with responses from the backend.

Note: This system closely mirrors the architecture of [Custom Message Rendering](/chat/custom-message-rendering).
