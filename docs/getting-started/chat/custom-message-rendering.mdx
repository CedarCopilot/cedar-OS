...
| ---------- | ------------------------------------------- | ----------- | --------------------------------------------------------------------------------- |
| `validate` | Structured object of broader type `Message` | `boolean`   | Decides whether incoming object of this type should be handled by this processor. |
...
```typescript
import { useActionMessageProcessor, ActionMessageFor } from 'cedar-os';

// Step 1: Define the custom message type using the helper
type TodoActionMessage = ActionMessageFor<
	'todos', // state key
	'addTodo', // setter key
...
// Step 3: Register the processor with full control
useActionMessageProcessor<TodoActionMessage>({
	namespace: 'todos',
