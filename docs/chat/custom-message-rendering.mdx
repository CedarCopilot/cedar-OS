---
title: 'How Messages are Rendered (and Custom Rendering)'
...
Note: This system closely mirrors the architecture of [Custom Response Processing](/agent-backend-connection/custom-response-processing).
...
- **Default Renderer**: `ActionRenderer` shows action completion with shimmer effects

**Action Factory Functions**: Cedar provides special factory functions for action messages:

```tsx
import { createActionMessageRenderer, ActionMessageFor } from 'cedar-os';

// Specific action message type using helper
type UpdateCounterAction = ActionMessageFor<
	'counter', // StateKey
	'increment', // SetterKey
	[number] // Args
>;

// Create custom action renderer with filtering
const CounterActionRenderer = createActionMessageRenderer<UpdateCounterAction>({
	namespace: 'counter',
