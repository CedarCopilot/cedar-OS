...
- Use 'defaultUser' as the user ID
- Load any existing threads for the default user
- Select the first thread or creates a 'defaultThread' thread
...
function App() {
	return (
		<CedarCopilot
			messageStorage={{
				type: 'remote',
				// All endpoints are prefixed with '/chat' (e.g., {baseURL}/chat/threads)
				options: {
...
| ------ | -------------------------------------------------------- | ----------------------------- | -------------------- | -------------- |
| GET    | `{baseUrl}/chat/threads`                                 | List threads (optional)       | -                    | `ThreadMeta[]` |
| POST   | `{baseUrl}/chat/threads`                                 | Create new thread (optional)  | `{threadId, meta}`   | -              |
| GET    | `{baseUrl}/chat/threads/{threadId}`                      | Get thread messages           | -                    | `Message[]`    |
| PUT    | `{baseUrl}/chat/threads/{threadId}`                      | Update thread meta (optional) | `{meta: ThreadMeta}` | -              |
| DELETE | `{baseUrl}/chat/threads/{threadId}`                      | Delete thread (optional)      | -                    | -              |
| POST   | `{baseUrl}/chat/threads/{threadId}/messages`             | Add single message            | `{message: Message}` | -              |
| PUT    | `{baseUrl}/chat/threads/{threadId}/messages/{messageId}` | Update message (optional)     | `{message: Message}` | -              |
| DELETE | `{baseUrl}/chat/threads/{threadId}/messages/{messageId}` | Delete message (optional)     | -                    | -              |
...
| Method | Endpoint                                                             | Purpose                       | Request Body         | Response       |
| ------ | -------------------------------------------------------------------- | ----------------------------- | -------------------- | -------------- |
| GET    | `{baseUrl}/chat/threads?userId={id}`                                 | List threads (optional)       | -                    | `ThreadMeta[]` |
| POST   | `{baseUrl}/chat/threads?userId={id}`                                 | Create new thread (optional)  | `{threadId, meta}`   | -              |
| GET    | `{baseUrl}/chat/threads/{threadId}?userId={id}`                      | Get user's thread messages    | -                    | `Message[]`    |
| PUT    | `{baseUrl}/chat/threads/{threadId}?userId={id}`                      | Update thread meta (optional) | `{meta: ThreadMeta}` | -              |
| DELETE | `{baseUrl}/chat/threads/{threadId}?userId={id}`                      | Delete thread (optional)      | -                    | -              |
| POST   | `{baseUrl}/chat/threads/{threadId}/messages?userId={id}`             | Add message to user's thread  | `{message: Message}` | -              |
| PUT    | `{baseUrl}/chat/threads/{threadId}/messages/{messageId}?userId={id}` | Update message (optional)     | `{message: Message}` | -              |
| DELETE | `{baseUrl}/chat/threads/{threadId}/messages/{messageId}?userId={id}` | Delete message (optional)     | -                    | -              |
...
**Setting User ID:**
User ID is treated internally as just another Cedar State.
