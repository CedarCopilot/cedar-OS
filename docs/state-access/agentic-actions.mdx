...
## Automatic Setter Schema Distribution

When you subscribe a state to input context using `useSubscribeStateToInputContext`, Cedar automatically includes the schemas of all custom setters for that state in the context sent to your agent. This enables your agent to understand what actions are available and their parameter requirements.

### Setter Schema Structure

The setter schemas are included in a top-level `setters` field alongside your subscribed state data:
...
```json
{
	...
	"setters": {
		"addNode": {
			...
			"schema": {
				// Zod schema converted to JSON Schema format
			}
		},
		"removeNode": {
			"name": "removeNode",
...
If your custom setters include a `schema` field with a Zod schema, Cedar will automatically convert it to JSON Schema format using `zod-to-json-schema` and include it in the setter information. This provides your agent with precise type information for generating valid parameters.
...
This automatic schema distribution ensures your agent always has up-to-date information about available actions and their requirements, enabling more accurate and reliable action execution.
